...
| D6 | **Grafana is Grafana CLOUD** (no in-cluster Grafana pod/svc/ingress; ESO grafana generator + Grafana Cloud remote_write creds confirm it). Dashboard shipped as **versioned JSON**. **OPEN — D-GRAFANA, human sign-off [R4, eng-review]:** provisioning path = **(a)** Grafana Terraform provider (`grafana_dashboard`, token from Vault) — *recommended, IaC-consistent* — vs. **(b)** commit JSON + import via the existing ESO grafana generator (no new provider/token). | Matches how the stack already integrates with Grafana Cloud (Prometheus remote_write, ESO SA-token generator). Left OPEN rather than silently spending a new-provider + Vault-token token. If Grafana is ever self-hosted, the same JSON drops into a ConfigMap sidecar. |
...
**UNRESOLVED:** 1 — **D-GRAFANA** (dashboard provisioning: Grafana TF provider vs. ESO-import). Recommendation: TF provider (IaC-consistent). Needs human sign-off before S2 finalizes.
**VERDICT:** ENG CLEARED — additive *wiring* plan grounded in live discovery. Two reality
corrections stand: (1) **Grafana is Grafana Cloud, not self-hosted**; (2) a **collector already
exists + traces already flow to Grafana Cloud Tempo**. Eng review caught two landmines the
first draft missed — the invalid shared-Service patch (R1) and the empty-RED-metrics source
(R2) — both now fixed; the plan touches **no shared live resource** (PodMonitor is fully
additive). Traces (D4): keep Grafana Cloud Tempo; self-hosted Tempo flagged OPEN. Awaiting
human sign-off of **D-GRAFANA** + flip of `status: draft → approved` + `approved_by`/`approved_at`
before S1.
