...
- **OpenAPI spec + Swagger UI + tbls ERD + mermaid sequences + docs CI gate (former S18–S22)** — deferred
  to **plan 02 (docs-hardening)** per eng-review scope reduction (2026-05-28). AT12, IT6, IT10, IT11
  move with them. S17's contract tests use hand-built golden fixtures until plan 02 tightens to a
  spec-driven oracle. Risk owned: spec-vs-code drift window between plan 01 land and plan 02 land.
...
## API Conventions (locked plan 01 — applied to every endpoint)
...
| IT6 | _Deferred to plan 02 (openapi.yaml not in this plan) — S17 uses hand-built golden response fixtures until then_ | _internal/handler/contract_test.go (plan 02 tightens to spec-driven)_ |
...
| IT10| _Deferred to plan 02 (docs-hardening) — tbls ERD generation lives there_ | _ci: docs job (plan 02)_ |
| IT11| _Deferred to plan 02 (docs-hardening) — depends on openapi.yaml landing first_ | _internal/handler/openapi_coverage_test.go (plan 02)_ |
...
## Technical documentation (deferred to plan 02 — docs-hardening)

Plan 01 ships handlers + tests + Dockerfiles only. The full spec-first docs stack (OpenAPI + oapi-codegen,
tbls ERD + diff gate, mermaid sequence diagrams, Swagger UI at /docs, graphify re-index) moves to
**plan 02 (docs-hardening)**. Until then, S17's contract tests rely on hand-built golden response fixtures —
intentional spec-vs-code drift window, owned by the docs-hardening plan.

(Original spec-first table moved to plan 02. Source reference diagrams to port intent from:
...
(AT12, IT6, IT10, IT11, Swagger UI, openapi.yaml, tbls ERD → plan 02.)

## NOT in scope (plan 01)
...
- OpenAPI spec, Swagger UI, oapi-codegen handler types — **deferred to plan 02 (docs-hardening)**.
- tbls ERD generation + `tbls diff` CI gate — **deferred to plan 02**.
- Mermaid sequence diagrams in `docs/sequences/` (this plan ships ASCII diagrams inline) — **plan 02**.
- graphify re-index CI step — **plan 02**.
...
- Plan 00 landed (`cade037`, `cbc247f`): `go.mod` with pinned deps, sqlc.yaml, golang-migrate, Makefile
  ...
  `internal/`, `cmd/`, or `db/query/` yet — plan 01 is greenfield Go.
...
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | CLEAR (PLAN) | 8 issues raised + 10 API-surface findings; 16 implementation tasks queued; scope reduced (S18–S22 → plan 02) |
